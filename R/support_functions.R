...
scto_other_specify <- function(input_tool_data,
    ...
    df_join_other_response_with_choices <- df_data_parent_qns %>%
        ...
        filter(str_detect(string = current_value, pattern = "98\\b|\\w+_98\\b"))
    ...
    output$select_mu_data <- df_join_other_response_with_choices %>%
        ...
        mutate(rank = row_number(),
               i.check.question = case_when(rank == 1 ~ other_name,
                                            rank == 2 ~ paste0(name, "_", int.my_current_val_extract),
                                            rank == 3 ~ paste0(name, "_")),
               i.check.old_value = case_when(rank == 1 ~ as.character(other_text),
                                             rank == 2 ~ "1",
...
# other specif for repeats

ctso_other_specify_repeats <- function(input_repeat_data,
